...
>
> One test did slip through both layers on the first run. I added a simple protection, re-ran the test, and it was blocked. The failure, fix, and re-run are all documented — no hand-waving.
>
> The agent also tried to generate infrastructure code that didn't meet basic safety checks, and the pipeline refused to approve it. Nothing moves forward unless it passes the gate.
>
...
>
> Built with an AI coding assistant (Claude Code) — it helped write the pipeline/report tooling
> and drafted this write-up; every number here was independently checked against raw run data
> before publishing.
>
